...
import { parseMission, type MissionModelClient } from "./mission-parser";
...
function recordSpace(spaceId: string, senderId: string | undefined): void {
  ...
  const known: Record<string, { senderId?: string; lastSeen: string }> = existsSync(SPACES_FILE)
...
type SpaceSession = { pendingImageUrl?: string; cardMessage?: Message };

export async function runAgentLoop(app: SpectrumApp, deps: AgentLoopDeps): Promise<void> {
...
async function handleInbound(
  ...
  if (content.type === "attachment") {
    ...
    await space.send("Got the photo — tell me the goal, budget, and any must-haves.");
    return;
  }
  ...
  await space.responding(async () => {
    ...
    const parsed = await parseMission({ text, imageUrl }, current, deps.client);

    switch (parsed.intent) {
      case "smalltalk":
        await space.send(
          parsed.reply ?? "Hi! Text me a shopping goal with a budget and I'll assemble the kit.",
        );
        return;
      ...
      case "revise": {
        if (!parsed.mission) {
          await space.send(parsed.reply ?? askForMissing(parsed.missingFields));
          return;
        }
  ...
  return {
    ...
    slots: parsed.slots.map((slot) => {
      ...
      const sameSpec =
        ...
        JSON.stringify(slot.hardConstraints) === JSON.stringify(liveSlot.hardConstraints) &&
        JSON.stringify(slot.softPreferences) === JSON.stringify(liveSlot.softPreferences);
...
async function runMissionSearch(
  ...
  const mission =
    isNewMission || !previous ? parsedMission : mergeLiveSlotState(parsedMission, previous);
  ...
  const slotsToSearch = mission.slots.filter((slot) => !slot.locked && slot.candidates.length === 0);
  ...
  await mapWithConcurrency(slotsToSearch, SEARCH_CONCURRENCY, async (slot) => {
    try {
      ...
      deps.store.dispatch(space.id, {
        ...
        slotId: slot.id,
        candidates: found.map((candidate) => annotateConstraints(candidate, hard)),
      });
    } catch (error) {
  ...
  if (bundle.status === "ready") {
    ...
    for (const [slotId, variantId] of Object.entries(bundle.selections)) {
      deps.store.dispatch(space.id, { type: "candidateSelected", slotId, variantId });
    }
  }
  await refreshCard(session, cardUrl);
  ...
  await space.send(bundleSummary(deps.store.getBySpace(space.id) ?? latest, bundle));
}

async function handleCheckout(space: Space, deps: AgentLoopDeps, reply?: string): Promise<void> {
  const mission = deps.store.getBySpace(space.id);
  if (!mission) {
    await space.send(reply ?? "There's no active mission yet — text me a goal and budget first.");
    return;
  }
  ...
  try {
    plan = await buildCheckoutPlan(mission, { createMerchantCarts: deps.createMerchantCarts });
  } catch (error) {
  ...
  await space.send(appCard(`${deps.publicBaseUrl}/card/checkout/${mission.id}`, { live: true }));
  await space.send(checkoutSummary(plan, mission.budget.currency));
}

// ---------- helpers ----------

function askForMissing(missingFields: string[]): string {
  return missingFields.length > 0
    ? `One thing before I start: what's your ${missingFields.join(" and ")}?`
    : "Tell me a bit more about what you need.";
}

function ackLine(mission: ShoppingMission, isNewMission: boolean, searchCount: number): string {
...
function checkoutSummary(plan: CheckoutPlan, currency: string): string {
  const merchants = plan.merchants.length === 1 ? "1 merchant" : `${plan.merchants.length} merchants`;
  ...
  const changed = plan.priceChanges.length === 1 ? "1 price" : `${plan.priceChanges.length} prices`;
...
async function refreshCard(session: SpaceSession, cardUrl: string): Promise<void> {
  ...
  try {
    ...
    console.warn("[loop] card edit failed (card still updates on open):", error);
  }
}
...
function annotateConstraints(candidate: ProductCandidate, hard: string[]): ProductCandidate {
  if (hard.length === 0) return candidate;
  const haystack = [candidate.title, ...Object.values(candidate.selectedOptions)]
    .join(" ")
    .toLowerCase();
  const matched = [...candidate.matchedConstraints];
  const uncertain = [...candidate.uncertainConstraints];
  for (const constraint of hard) {
    if (matched.includes(constraint) || uncertain.includes(constraint)) continue;
    (haystack.includes(constraint.toLowerCase()) ? matched : uncertain).push(constraint);
  }
  return { ...candidate, matchedConstraints: matched, uncertainConstraints: uncertain };
}
